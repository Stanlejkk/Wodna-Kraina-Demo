...
export function FishCard({ fish, variant = "small" }: FishCardProps) {
  ...
  return (
    <div
      className={`card-hover flex flex-col h-full rounded-lg border border-border bg-card overflow-hidden ${
        isLarge ? "p-6 lg:p-8" : "p-5"
      }`}
    >
      {/* Fish icon area */}
      <div
        className={`relative rounded-md mb-4 flex items-center justify-center shrink-0 ${
          isLarge ? "h-40" : "h-28"
        }`}
        style={{
      ...
      >
        ...
        >
          <path
            ...
            fillOpacity="0.25"
          />
          <circle cx="75" cy="25" r="3" fill={fish.color} fillOpacity="0.4" />
        </svg>
      ...
      <p className={`text-foreground/50 leading-relaxed flex-1 ${isLarge ? "text-sm" : "text-xs"}`}>
        {fish.description}
      </p>
    </div>
  );
